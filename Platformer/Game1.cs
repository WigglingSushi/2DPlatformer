...
namespace Platformer
{
    ...
    {
        string next_level = @"Content\\lev2.txt";

        // CHANGE THIS FOR MAKING A NEW LEVEL                      (use backup to retrieve a copy of old one if you accidentally save over it)
        static public string LEVEL_NAME  = @"Content\\lev3.txt";   // this is stored in the BIN/execute version of Content
        static public string BACKUP_NAME = @"Content\\backup.txt"; 

        // GAME STATES:
        enum GameState { edit, play, game_over, menu, load_level, game_win }
        ...
        private void Start(string name_level)
        {
            // LOAD GRAPHICS: 
            far_background = Content.Load<Texture2D>("bg");
            //mid_background = Content.Load<Texture2D>("mid_background");
            tiles_image = Content.Load<Texture2D>("tiles2");
            ...
            // CREATE PLAYER
            player = new Player(screen_center, quadBatch, map, inp, hud, explodeSys, sheet, editor); // add explosions, etc.. 

            // CREATE MONSTER SYSTEM
            monsterSys = new MonsterSys(Content, quadBatch, map, player, explodeSys);
            monsterSys.Load(1, Content);

            player = new Player(screen_center, quadBatch, map, inp, hud, explodeSys, sheet, editor, monsterSys);
            ...
            player.Load(meo, tiles_image);
                ...
                if (File.Exists(name_level))
                {
                    editor.LoadLevel(name_level);
                }
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            switch (gameState)
            {
                case GameState.play: 
                    ...
                    Vector2 cam_topHalf =new Vector2(player.pos.X,player.pos.Y / 2);

                    cam_pos = cam_pos + (cam_topHalf - cam_pos) * 0.2f;               // smooth pan camera toward player's world position (ease-in) 
                ...
                case GameState.game_over:
                    // GAME OVER
                    if (inp.Keypress(Keys.Escape) || inp.Keypress(Keys.Enter)) Start(LEVEL_NAME); // change to go to menu state later
                    break;
            }
